...
export async function generateInvoicePDF(
  ...
  return new Promise((resolve) => {
    ...
    if (project.type === "daily_rate") {
      ...
      sortedWorkdays.forEach((workday) => {
        ...
        if (!printedDates.has(formattedDate)) {
          ...
          doc.text(`Daily Rate - ${formattedDate}`, descriptionX, currentY);
          doc.text(formatCurrency(project.rate), rateX, currentY, {
            width: 70,
            align: "center",
          });
          doc.text("1", quantityX, currentY, { width: 40, align: "center" });
          doc.text(formatCurrency(project.rate), amountX, currentY, {
            width: 70,
            align: "right",
          });
          currentY += 25;
    ...
    doc.text(
      invoice.notes ||
        "Thank you for your business! Payment is due within 14 days of invoice date.",
    );
